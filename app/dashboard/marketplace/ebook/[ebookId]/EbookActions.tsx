...
export default function EbookActions({ 
  ...
  const handleAffiliate = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/affiliate/ebook/join', {
        method: 'POST',
        headers: {
      ...
      {isAffiliated ? (
        <div className="space-y-2">
          <p className="text-sm font-medium text-green-600">
            Você é afiliado deste eBook
          </p>
          <div className="relative">
            <input
              type="text"
              value={`${process.env.NEXT_PUBLIC_APP_URL}/ebooks/${ebookId}?ref=${affiliateCode}`}
              readOnly
            ...
            <button
              onClick={() => {
                navigator.clipboard.writeText(
                  `${process.env.NEXT_PUBLIC_APP_URL}/ebooks/${ebookId}/checkout?ref=${affiliateCode}`
                )
